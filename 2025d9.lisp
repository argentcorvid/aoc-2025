...
(defun valid-rectangle-p (rect-corner-pair green-edges all-red-tiles)
  (let* ((rect-edges (get-rectangle-edges rect-corner-pair))
         (all-corners (mapcar #'first rect-edges)))
    (not (or (some (a:rcurry #'point-inside-shape-p rect-edges) all-red-tiles)
             (and (notany (a:rcurry #'point-inside-shape-p green-edges) all-corners)
                  (notany (a:rcurry #'point-on-edge-p green-edges) all-corners))
             (some (lambda (rect-edge)
                     (some (a:curry #'edges-intersect-p rect-edge) green-edges))
                   rect-edges)))))
  ...
  (let* ((green-edges (sort (get-shape-edges red-tiles) #'> :key #'edge-length)) ;longer edges are more likely to intersect
    ...
    (let* ((filtered (remove-if-not (lambda (rect)
           ...
           (largest-valid (find-if (lambda (rect)
                                     ...
                                     (valid-rectangle-p rect green-edges red-tiles))
                                  ; (sort filtered #'> :key #'second )
                                   (sort rectangles #'> :key #'second)
                                   :key #'first)))
      (apply #'format t "found largest vaild: ~a area ~a" largest-valid)
