...
(defun print-grid (in-list)
  (let* ((min-row 0)
         (min-col 0)
         (max-row (+ 2 (reduce #'max in-list :key #'second)))
         (max-col (+ 2 (reduce #'max in-list :key #'first)))
...
(defun shrink-rectangle (corner-pair)
  (destructuring-bind ((c1 r1)
    ...
    (cond ((and (/= c1 c2)
                (/= r1 r2))
           (list (list (1+ (min c1 c2)) (1+ (min r1 r2)))
                 (list (1- (max c1 c2)) (1- (max r1 r2)))))
          ((= c1 c2)
           (list (list c1 (1+ (min r1 r2)))
                 (list c1 (1- (max r1 r2)))))
          (t
           (list (list (1+ (min c1 c2)) r1)
                 (list (1- (max c1 c2)) r1)))
        )))
...
(defun p2 (red-tiles)
  (let* ((rectangles (list)))
    ...
    (dolist (rect (sort rectangles #'> :key #'second))
      (let ((rect-bounds (get-shape-edges (shrink-rectangle (first rect)))))
       (when  (notany (a:rcurry #'point-inside-shape-p rect-bounds) 
                     red-tiles)
         (return (second rect)))))))
